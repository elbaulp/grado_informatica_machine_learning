...
y los negativos ($3$ y $6$ respetivamente).

El error de test que hemos obtenido es del $10.38961\%$, lo cual 
...
$5$. Con este valor para `k` se ha obtenido la siguiente matriz de confusión:
...
Y un error de test de $5.194805\%$, el cual es bastante bueno.
...
pred.object.knn <- ROCCurve(pred, test$mpg01, main = "ROC curve for KNN with k = 5")
pred.object.glm <- ROCCurve(glm.pred, test$mpg01, main = "ROC curve for GLM")
...
En concreto, $AUC_{knn} = 0.9142857$ y $AUC_{glm} = 0.8714286$

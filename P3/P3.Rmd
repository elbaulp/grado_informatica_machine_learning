...
pred.object.knn <- ROCCurve(prob, test$mpg01, col = "blue", lwd = 2)
pred.object.glm <- ROCCurve(glm.probs, test$mpg01, add = T, lwd = 2, col = "red")
...
# data <- data.frame(M = prob, D = test$mpg01)

# ggplot(data, aes(d = D, m = M)) + ?geom_roc()

# Hay que extraer la probabilidad del knn con prob, y luego restar 1-prob para 
# poder dibujar la curva roc bien.

...
Para $d=3$ (dimensión) generar un conjunto de $N$ datos aleatorios $\{\textbf{x_n},y_n\}$
de la siguiente forma. Para cada punto $\textbf{x_n}$ generamos sus coordenadas
muestreando de forma independiente una ${\mathcal{N}}(0,1)$. De forma similar 
generamos un vector de pesos de (d+1) dimensiones $\textbf{w_f}$, y el conjunto
de valores $y_n=\textbf{w_f}^T\textbf{x_n}+\sigma\epsilon_n$, donde $\epsilon_n$
es un ruido que sigue también una ${\mathcal{N}(0,1)$ y $\sigma^2$ es la varianza del ruido; 
fijar $\sigma=0.5$.

Usar regresión lineal con regularización ``weight decay'' para estimar $\textbf{w_f}$
con $\textbf{w_{reg}}$. Fijar el parámetro de regularización a $0.05/N$.

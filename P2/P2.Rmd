...
Newton <- function(initial.point = c(1,1), 
  ...
  while (nIters < maxIters && !threshold) {
    ...
    # norm(old.values - new.values, type = "F")
    # falta ponerle el abs(fyold - fynew)
    if (norm(derivada en el punto actual, type = "F") < 1e-5)
      threshold <- T
...
normalized.p.list <- legendre.polynomials(10, normalized = T)
# L. Polynomials with coefficients
lY =  sapply (x, function(x) polynomial.values( normalized.p.list,x))
fn = matrix(unlist(lY), nrow = N, byrow = T)
yn <- fn + sqrt(sigma2)*rnorm(N)
```

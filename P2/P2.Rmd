...
ReadDigits <- function(dataset){
  ...
  digits <- read.table(dataset,
                       quote = "",
                       comment.char = "",
                       stringsAsFactors = F)
  
  digits <- digits[digits$V1 == 1 | digits$V1 == 5, ]
  labels <- digits[,1]
  digits <- digits[,-1]
  digits <- lapply(1:nrow(digits), function(i){matrix(digits[i, ], nrow = 16)})
  
  # Compute mean and symmetry
  digits.mean <- sapply(1:length(digits), function(i) mean(as.double(digits[[i]])))
  
  # Compute vertical simmetry
  digits.inverted <- lapply(1:length(digits),
                          function(i) digits[[i]][, 16:1])
  digits.vertical.symmetry <- sapply(1:length(digits),
                                   function(i) -sum(abs(as.numeric(digits[[i]])
                                               - as.numeric(digits.inverted[[i]]))))
  
  # Preparing data to apply Linear Regression
  digits <- data.frame(intensity = digits.mean, symmetry = digits.vertical.symmetry)
  
  list(digits = digits, labels = labels)
}
...
g2 <- AjustaPla(digits, label = labelscopy, vini = g$coefficients, memory = T)
...
```{r echo=F, Visualización de datos}
plot2 <- ggplot(digits, aes(intensity, symmetry)) + ggtitle("Train") +
  geom_point(aes(colour = factor(as.integer(labels))), shape = 19) + theme_bw() +

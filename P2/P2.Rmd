...
(Para una muestra nueva de $10000$ puntos):*

$$E_{out} = `r cross.entropy.error`$$

```{r Replicate 100 times}
data <- NewDataSet(1000)
cross.entropy.error <- replicate(100, {
  w <- SGD(data$x, data$y, h, dh, how.many.epochs = T)
  sum(parSapply(cl, data$y * (t(w$w) %*% t(data$x)), h))/nrow(data$x)
})
...
$$E_{out} = `r sum(cross.entropy.error)/nrow(data$x)`$$

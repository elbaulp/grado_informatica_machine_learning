...
*Como se aprecia en la tabla, la tasa de aprendizaje que se acerca más a un mínimo
...
```{r echo = F}
rm(cost, learning.ratio, nIters, u, v, w)
```


## Ejercicio 3

__Método de Newton__ Implementar el algoritmo de minimización de Newton y aplicarlo
...
__Regresión Logística__: En este ejercicio crearemos nuestra propia función
...
Consideremos $d=2$ para que los datos sean visualizables, y sea 
$\mathrm{\mathcal{X}=[-1,1]\times[-1,1]}$ con probabilidad uniforme de elegir cada
$\mathrm{\textbf{x}\in \mathcal{X}}$.  Elegir una línea en el plano como la frontera entre
$f(\textbf{x})=1$ (donde $y$ toma valores $+1$) y $f(\textbf{x})=0$ (donde $y$ toma valores
$-1$), para ello seleccionar dos puntos aleatorios del plano y calcular la línea
que pasa por ambos. Seleccionar $N=100$ puntos aleatorios $\{\textbf{x}_n\}$ de $\mathcal{X}$
...
- Implementar Regresión Logística(RL) con Gradiente Descendente Estocástico (SGD) bajo las siguientes condiciones:
  ...
  - Parar el algoritmo cuando $\mathrm{||\textbf{w}^{(t-1)}-\textbf{w}^{(t)}||<0.01}$, donde $\textbf{w}^{(t)}$ denota el vector de pesos al final de la época $t$. Una época es un pase completo a través de los $N$ datos.
...
- Usar la muestra de datos etiquetada para encontrar $g$ y estimar $E_{out}$  usando para ello un número suficientemente grande de nuevas muestras.
...
cl <- GetParCluster()
cross.entropy.error <- mean(parSapply(cl, data$y * (t(w) %*% t(data$x)), h))
stopCluster(cl)
```
...
- ({\em Opcional 0.2}) Repetir el experimento 100 veces con diferentes funciones frontera y calcule el promedio.

```{r Replicate 100 times}

# Export data needed for parallel processing
# clusterExport(cl, c("sgd.data"))

cl <- GetParCluster()
# clusterSetRNGStream(cl, iseed = 1000000007)
cross.entropy.error <- parSapply(cl, 1:100, function(x) {
  data <- NewDataSet(100)
  sgd.data <- cbind(data$x, as.vector(data$y))
  w <- SGD(sgd.data, h, dh, how.many.epochs = T)
  error <- sum(sapply(data$y * (t(w$w) %*% t(data$x)), h))/nrow(data$x)
...
- ¿Cuál es el valor de $E_{out}$ para $N=100$?
...
- ¿Cuántas épocas tarda en promedio RL en converger para $N=100$, usando todas las condiciones anteriormente especificadas?

*En cuanto a la media de épocas necesarias para converger, el valor es 
$`r mean(cross.entropy.error["epochs",])`$*

```{r echo = F}
rm(w, cross.entropy.error, data, line)
```
...
plot2 <- ggplot(digits.test, aes(intensity, symmetry)) + ggtitle("Test") +
...
- (\em{Opcional 0.1}) Repetir los puntos anteriores pero usando una transformación polinómica de tercer orden( $\Phi_3(\textbf{x})$ en las transparencias de teoría).

- (\em{Opcional 0.1}) Si tuviera que usar los resultados para dárselos a un potencial cliente ¿usaría la transformación polinómica? Explicar la decisión.

```{r echo = F}
rm(digits.test, digits.traninig, g2, w.pocket, x.test, x.training,
    e.in, e.test, g, labels.test, labelscopy.test, labelscopy, labels.training,
    line.g, line.g2, N, plot2, vc.generalization.bound.for.e.test, vc.generalization.bound.for.e.in)
```

# SobreAjuste

\textbf{Sobreajuste.} Vamos a construir un entorno que nos permita experimentar
con los problemas de sobreajuste. Consideremos el espacio de entrada $\mathcal{X}=[-1,1]$
con una densidad de probabilidad uniforme, $\mathbb{P}(x)=\frac{1}{2}$. Consideramos
dos modelos $\mathcal{H}_2$ y $\mathcal{H}_{10}$ representando el conjunto de todos
los polinomios de grado 2 y grado 10 respectivamente. La función objetivo es un
polinomio de grado $Q_f$ que escribimos como $f(x)=\sum_{q=0}^{Q_f} a_q L_q(x)$,
donde $L_q(x)$ son los polinomios de Legendre (ver la relación de ejercicios.2).
El conjunto de datos es $\mathcal{D}=\{ (x_1,y_1),\dots,(x_N,y_N)\}$ donde 
$y_n=f(x_n)+\sigma\epsilon_n$ y las $\{\epsilon_n\}$ son variables aleatorias i.i.d.
${\mathcal{N}}(0,1)$ y $\sigma^2$ la varianza del ruido.
...
Sean $g_2$ y $g_{10}$ los mejores ajustes a los datos usando $\mathcal{H}_2$ y
$\mathcal{H}_{10}$ respectivamente, y sean $E_{out}(g_2)$ y $E_{out}(g_{10})$ sus
respectivos errores fuera de la muestra.

-  Calcular $g_2$ y $g_{10}$

*Hemos generado un DataSet a partir de los polinomios de Legendre de grado 50, a
...
con un polinomio de Legendre de grado 50:*
...
(p + geom_line(aes(y = X.10.degree$predicted.from.g2, colour = "2nd Order Fit"))
   + geom_line(aes(y = X.10.degree$predicted.from.g10, colour = "10th Order Fit"))
...
(p + geom_line(aes(y = X.2.degree$predicted.from.g2, colour = "2nd Order Fit"))
   + geom_line(aes(y = X.2.degree$predicted.from.g10, colour = "10th Order Fit"))
   + scale_color_discrete(name = "Polynomial fits"))
```

*A partir de ahora el análisis realizado será sobre los datos generados a partir 
del polinomio de Legendre de grado 50, cuyos coeficientes para cada una de las 
estimaciones son:*
...
- ¿Por qué normalizamos $f$? (Ayuda: interpretar el significado de $\sigma$)
...
- ({\em Opcional 0.1})  ¿Cómo podemos obtener $E_{out}$ analíticamente para una $g_{10}$ dada ?

Siguiendo con el punto anterior, usando la combinación de parámetros
$Q_f=20,N=50,\sigma=1$  ejecutar un número  de experimentos (> 100) calculando 
en cada caso $E_{out}(g_2)$ y $E_{out}(g_{10})$. Promediar todos los valores de
error obtenidos para cada conjunto de hipótesis
...
cl <- GetParCluster(123456789)

result <- parSapply(cl, 1:100, function(x){
...
*Tras probar con estos datos, obtenemos un $E_{out}(\mathcal{H}_2) = `r eout.H2`$
y un $E_{out}(\mathcal{H}_{10}) = `r eout.H10`$. Es de esperar un valor mucho peor
para la hipótesis de $\mathcal{H}_{10}$, ya que estamos forzando mucho más el sobre
ajuste. Aunque el error dentro de la muestra sea mucho menor con respecto a $\mathcal{H}_2$,
...
(p + geom_line(aes(y = ds$g2, colour = "2nd Order Fit"))
   + geom_line(aes(y = ds$g10, colour = "10th Order Fit"))
   + scale_color_discrete(name = "Polynomial fits"))
```

*Como vemos, los picos del polinomio de grado $10$ perjudican mucho al error.*

Definimos una medida de sobreajuste como

$$E_{out}(\mathcal{H}_{10}) - E_{out}(\mathcal{H}_2)$$

- Argumentar por qué la medida dada puede medir el sobreajuste.
...
- ({\em Opcional 0.1}) Usando la combinación de valores de los valores $Q_f\in\{1,2,\dots,100\}$,
$N\in\{20,25,\dots,100\}$, $\sigma\in\{0;0.05;0.1;\dots;2\}$, se obtiene una gráfica
como la que aparece en la figura 4.3 del libro "Learning from data", capítulo 4.
Interpreta la gráfica respecto a las condiciones en las que se da el sobreajuste.

*Si se realiza el experimento para más valores de $Q_f, N ~ y ~ \sigma$ se obtiene
una perspectiva de cómo se comporta el sobreajuste en función del tamaño de la
muestra, del ruido y del grado de la función.*

*Si analizamos el sobre ajuste en función del tamaño de la muestra y el ruido, 
podemos concluir que el sobreajuste tiende a ser menor cuando el ruido es menor, o 
cuando el tamaño de la muestra crece. Por lo general hay una relación lineal
entre ruido y tamaño de la muestra en cuanto sobre ajuste. Esta “linealidad”
también se debe a que se han normalizado los datos.*

*Si miramos desde el punto de vista de la complejidad de la función $f$, vemos como
esta complejidad está relacionada con el sobreajuste también, aunque no de forma
lineal. Como conclusión podríamos decir:*

- *Si el número de muestras aumenta, reducimos el sobreajuste.*
- *Si el ruido estocástico (Variando $\sigma$) aumenta, el sobreajuste aumenta.*
- *Si el ruido determinísta aumenta (Incrementamos la complejidad de la hipótesis $\mathcal{H}_{10}, \mathcal{H}_{20}, \dots$), el sobreajuste aumenta.*

```{r echo = F}
rm(ds, X.Qf.degree, X.2.degree, X.10.degree, eout.H10.residuals, eout.H2.residuals,
   eout.H2, eout.H10, model.g10, model.g2, model.g10.from.degree2, model.g2.from.degree2,
   N, p, Qf, sigma2)
```
...
Para $d=3$ (dimensión) generar un conjunto de $N$ datos aleatorios $\{\textbf{x}_n,y_n\}$
de la siguiente forma. Para cada punto $\textbf{x}_n$ generamos sus coordenadas
muestreando de forma independiente una ${\mathcal{N}}(0,1)$. De forma similar 
generamos un vector de pesos de (d+1) dimensiones $\textbf{w}_f$, y el conjunto
de valores $y_n=\textbf{w}_f^T\textbf{x}_n+\sigma\epsilon_n$, donde $\epsilon_n$
es un ruido que sigue también una $\mathcal{N}(0,1)$ y $\sigma^2$ es la varianza del ruido; 
fijar $\sigma=0.5$.

Usar regresión lineal con regularización ``weight decay'' para estimar $\textbf{w}_f$
con $\textbf{w}_{reg}$. Fijar el parámetro de regularización a $0.05/N$.
...
Cross.Validation <- function(data, lambda = .05) {
  # Cross Validation
  #
  # Args:
  #   data: Data
...
- Para $N\in\{d+15,d+25,\dots,d+115\}$ calcular los errores $\textbf{e}_1,\dots,\textbf{e}_N$ de validación cruzada y $E_{cv}$.
...
cl <- GetParCluster()
cross.validation.errors <- parSapply(cl, N, Cross.Validation)
stopCluster(cl)
# Compute $E_{cv}$
...
- Repetir el experimento $10^3$ veces, anotando el promedio y la varianza de $\textbf{e}_{1}$, $\textbf{e}_{2}$ y $E_{cv}$ en todos los experimentos.

```{r 1.b}
# Replicate the above experiment 10^3 times
run.x.times <- 1000
cl <- GetParCluster()
result <- parSapply(cl, 1:run.x.times, function(x) {
...
- ¿Cuál debería de ser la relación entre el promedio de los valores de $\textbf{e}_{1}$ y el de los valores de $E_{cv}$ ? ¿y el de los valores de $\textbf{e}_{2}$? Argumentar la respuesta en base a los resultados de los experimentos.
...
*Como resultado de todo esto, tenemos que el error de validación cruzada tiende a 
ser cercano al de fueda de la muestra, es decir $E_{out}\approx E_{cv}$*

- ¿Qué es lo que contribuye a la varianza de los valores de $\textbf{e}_{1}$ ?
...
- Si los errores de validación-cruzada fueran verdaderamente independientes, ¿cual sería la relación entre la varianza de los valores de $\textbf{e}_{1}$ y la varianza de los de $E_{cv}$?

*Si los errores de validación cruzada fueran independientes, una alta varianza
en estos no tendría por qué darnos una alta varianza en $E_{cv}$. El problema 
de usar para validación datos que vamos a usar para entrenamiento nos influirá en 
estos resultados, a pesar de esto, obtenemos una buena estimación porque entrenamos
$N-1$ modelos y validamos cada uno de ellos, luego estadísticamente tendremos una
buena aproximación de cómo se comporta el modelo fuera de la muestra.*

- Una medida del número efectivo de muestras nuevas usadas en el cálculo de $E_{cv}$ es el cociente entre la varianza de $\textbf{e}_{1}$ y la varianza de $E_{cv}$. Explicar por qué, y dibujar, respecto de $N$, el número efectivo de nuevos ejemplos($N_{eff}$) como un porcentaje de $N$. NOTA: Debería de encontrarse que $N_{eff}$ está cercano a $N$.
...
*Al aumentar la regularización, estamos reduciendo la varianza del error, ya que
hacemos que el modelo sea “más suave”, por ejemplo, al aumentar regularización
en un polinomio de grado $10$ podremos llegar a conseguir que parezca una curva de 

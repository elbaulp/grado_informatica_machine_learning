...
SGD <- function(data, labels, h, dh, tolerance = 0.01, learning.rate = 0.01) {
  ...
  while (above.tolerance) {
    ...
    for (i in nrow(data)) {
      xi <- as.vector(data[i,])
      yi <- as.vector(labels[i])
      gradient <- dh(yi %*% t(xi), yi * (t(w) %*% xi))
    ...
    if (norm(w.old - w, type = "F") < tolerance) {
      above.tolerance <- F
    }
  }
  w
...
# Get a line to separate data
line <- GetLine()

N <- 100
x <- matrix(runif(N*2, -1,1), N, 2)
y <- as.vector(LabelData(x, line$points))
x <- cbind(rep(1, N), x)
...
```{r echo=F}
label <- factor(as.integer(y))
ggplot(data = as.data.frame(x[,-1]), aes(V1, V2)) + geom_point(aes(colour = label)) + theme_bw()
```

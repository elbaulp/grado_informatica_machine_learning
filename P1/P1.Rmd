...
set.seed(1000000007)
prompt <- "Press <return> to continue to the next exercise"
```
...
error.10.iters <- sum(as.numeric(sign(t(w.10.iters) %*% t(perceptron.data.noisy))) != data.fsign.noisy[3])
error.100.iters <- sum(as.numeric(sign(t(w.100.iters) %*% t(perceptron.data.noisy))) != data.fsign.noisy[3])
error.1000.iters <- sum(as.numeric(sign(t(w.1000.iters) %*% t(perceptron.data.noisy))) != data.fsign.noisy[3])
...
w.100.iters <- as.matrix(unlist(ws[,2]))
w.1000.iters <- as.matrix(unlist(ws[,3]))
perceptron.data <- cbind(rep(1, nrow(perceptron.data)), perceptron.data)

error.10.iters <- sum(as.numeric(sign(t(w.10.iters) %*% t(perceptron.data))) != data.functions$F1)
error.100.iters <- sum(as.numeric(sign(t(w.100.iters) %*% t(perceptron.data))) != data.functions$F1)
error.1000.iters <- sum(as.numeric(sign(t(w.1000.iters) %*% t(perceptron.data))) != data.functions$F1)
...
mapply(AjustaPla, 
             max_iter = c(10, 100, 1000),
             MoreArgs = list(data           = perceptron.data.noisy[,-1],
                             label          = data.fsign.noisy[3],
                             visual         = T,
                             show.last.iter = T))
```
...
set.seed(1000000009)
w.no.memory <- AjustaPla(data           = perceptron.data.noisy[,-1],
                         label          = data.fsign.noisy[3],
                         visual         = T,
                         show.last.iter = T)
set.seed(1000000009)
w.memory <- AjustaPla(data           = perceptron.data.noisy[,-1],
                      label          = data.fsign.noisy[3],
                      visual         = T,
                      show.last.iter = T,
                      memory         = T)
...
ws <- mapply(AjustaPla, 
             max_iter = c(10, 100, 1000),
             MoreArgs = list(data           = as.matrix(data.functions[1:2]),
                             label          = data.functions$F1,
                             visual         = T,
                             show.last.iter = T,
                             memory         = T))

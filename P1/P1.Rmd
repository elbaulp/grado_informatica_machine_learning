...
w <- AjustaPla(x, y, visual = F)
DrawBoundary(w[-4], "blue")
...
```{r Exercise 2.3}
library(animation)
perceptron.data <- as.matrix(data.fsign.noisy[1:2])
y <- LabelData(perceptron.data)
MyPlot(perceptron.data, labels = y)
w = AjustaPla(perceptron.data, y, visual = T)

saveGIF({
  MyPlot(perceptron.data, labels = y)
  AjustaPla(perceptron.data, y, visual = T)
})

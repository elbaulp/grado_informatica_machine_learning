...
AjustaPla <- function(data,
  ...
  if (!converged) {
    ...
    if (show.last.iter == T && visual == T) {
      DrawBoundary(vini, "#D2372B", lwd = 3)
      legend('bottomright',
    ...
    DrawBoundary(vini, "#F9CE0C", lwd = 3)
    legend('bottomright',
           c("Final", "PLA History"),
           lty = c(1,1),
           lwd = c(2.5,2.5),
...
DrawBoundary(w.1000.iters, 'green')

# MyPlot(perceptron.data, data.functions$F1)
